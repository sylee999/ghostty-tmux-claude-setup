...
- **`파일:라인` 형태 클릭 시 라인 점프 (`docs/foo.md:285`)** — Ghostty 공식
  한계 ([discussion #11378](https://github.com/ghostty-org/ghostty/discussions/11378)).
  Ghostty 의 path detection regex 가 `:line` suffix 를 경로의 일부로 인식하지
  않음. 라인 번호 없는 경로(`docs/foo.md`) 는 클릭 시 시스템 기본 핸들러로
  정상 오픈. 업스트림에 `link-opener` config 옵션이 제안됐으나 아직 머지 안 됨.
  우회: Claude Code 의 파일 편집 도구 직접 사용, 또는 경로를 복사해 터미널에서
  `$EDITOR -g path:line` 수동 실행.
